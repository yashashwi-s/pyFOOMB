...
\begin{frame}[plain]
  ...
  \begin{center}
    ...
    {\large\color{accentblue} BC-392 : UG Project - 1 Semester Report}\\[0.8cm]
    \begin{tabular}{rl}
      \textbf{Student:} & Yashashwi Singhania (23014019) \\
      \textbf{Guide:} & Dr.\ Sharon Mano Pappu J. \\
      \textbf{Department:} & Biochemical Engineering \\
      \textbf{Lab:} & Bioprocess Development and Digitalization Laboratory \\
    \end{tabular}\\[0.5cm]
    {\small April 2026}
  \end{center}
  \vfill
...
\begin{frame}{Project Overview}
  \begin{block}{Objective}
    ...
    by exploiting the relationship between specific product formation rate ($q_p$)
    and specific growth rate ($\mu$).
  \end{block}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item[\textcolor{donegreen}{\checkmark}] Surveyed $q_p$--$\mu$ models from literature
...
% ── Slide 3: Why Fed-Batch? Why qp vs mu? ─────────────────
\begin{frame}{Why Optimise Fed-Batch Using $q_p$ vs $\mu$?}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \textbf{Recombinant Protein Production:}
      \begin{itemize}
        \item In recombinant organisms, \textbf{growth} and \textbf{product formation}
              are governed by different cellular mechanisms.
        \item Faster growth ($\uparrow\mu$) does not always mean more product ($\uparrow q_p$).
        \item Overexpression of foreign proteins competes with cell maintenance,
              causing stress at high $\mu$.
        \item The $q_p$--$\mu$ relationship is product- and organism-specific.
      \end{itemize}

      \vspace{0.15cm}
      \textbf{Goal:} Find the $\mu$ that maximises $q_p$
      $\Rightarrow$ maximise volumetric productivity.
    \end{column}
    \begin{column}{0.44\textwidth}
      \textbf{Why Fed-Batch?}
      \begin{itemize}
        \item In fed-batch, substrate is added at a controlled rate.
        \item At quasi-steady state the \textbf{dilution rate} $D \approx \mu$.
        \item By controlling the feed rate $F$, we directly control $\mu$
              and therefore $q_p(\mu)$.
      \end{itemize}

      \vspace{0.15cm}
      \begin{block}{Control Strategy}
        \centering
        $F \;\rightarrow\; D \approx \mu \;\rightarrow\; q_p(\mu) \;\rightarrow\; P$
      \end{block}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      ...
      \begin{block}{}
        ...
        Users implement \textit{any} $q_p$--$\mu$ relationship
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item[\textcolor{donegreen}{\checkmark}] 8 pre-built substrate kinetic templates
...
\begin{frame}{GUI: Modelling Workflow}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Substrate Kinetic Templates (8):}
      {\small
      \begin{itemize}
...
% ── Slide 8: Substrate Kinetic Model Equations ─────────────
\begin{frame}{Substrate Kinetic Model Equations}
  {\small All 8 pre-built templates model growth rate $\mu$ as a function of substrate $S$:}
  ...
  \begin{tabular}{l>{$\displaystyle}l<{$}}
    ...
    Andrews & \mu = \mu_{max} \frac{S}{K_S + S + S^2/K_I} \\
    Logistic & \mu = \mu_{max} \left(1 - \frac{X}{X_{max}}\right) \\
    Contois & \mu = \mu_{max} \frac{S}{K_{SX} X + S} \\
    Double Monod & \mu = \mu_{max} \frac{S_1}{K_{S1}+S_1} \cdot \frac{S_2}{K_{S2}+S_2} \\
    Exp.\ Growth & \mu = \mu_{max} \quad \text{(unlimited)} \\
    Exp.\ Decay & \mu(t) = \mu_0 \, e^{-k_d t} \\
    Fed-Batch Monod & \text{Monod} + \frac{dV}{dt}=F(t),\; D=F/V \\
    \bottomrule
  \end{tabular}
  }
  \end{center}
  ...
  \vspace{0.05cm}
  {\small These substrate kinetics define $\mu$, which then drives $q_p(\mu)$ for product formation.}
\end{frame}

% ── Slide 9: GUI Screenshot ───────────────────────────────
...
% ── Slide 9: qp–μ Relationship Models ─────────────────────
\begin{frame}{$q_p$--$\mu$ Relationship Models}
  The $q_p$--$\mu$ relationship governs how fast a cell produces product at a given growth rate.
...
\begin{frame}{Literature Survey: 9 Products, 2 Organisms}
  \vspace{-0.1cm}
  {\small $q_p$--$\mu$ data extracted from chemostat (CSTR) steady-state experiments at various dilution rates.}
...
\begin{frame}{Extracted Data: $q_p$ and $\mu$ Ranges}
  \vspace{-0.1cm}
...
\end{frame}
% ── Slide 12: qp–μ Plot ──────────────────────────────────
\begin{frame}{$q_p$--$\mu$ Fitting Results}
  \vfill
  \begin{center}
...
\begin{frame}{Batch X, S, P Profile Simulation}
  From the fitted $q_p$--$\mu$ parameters, we simulated full batch dynamics for each product.
...
\begin{frame}{Project Roadmap}
  ...
  \begin{tikzpicture}[
    ...
    \node[doneblock] (a4) at (9,0) {$q_p$--$\mu$ Fitting};
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item[\textcolor{donegreen}{\checkmark}] 9-product $q_p$--$\mu$ literature survey
